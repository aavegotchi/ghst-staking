...
async function main() {
  ...
  let signer;
  const owner = await (
    await ethers.getContractAt("OwnershipFacet", ghstStakingDiamondAddress)
  ).owner();
  ...
  const StkGHSTWETH = await StkGHSTWETHFactory.deploy(
  ...
  const diamondCut = (
    await ethers.getContractAt("IDiamondCut", ghstStakingDiamondAddress)
  ).connect(signer);
  let tx;
  let receipt;
  ...
  facet = (
    await ethers.getContractAt("StakingFacet", ghstStakingDiamondAddress)
  ).connect(signer);
  const ghstWethToken = "0xccb9d2100037f1253e6c1682adf7dc9944498aff";
  // 20 percent more than 1 GHST per day

  if (testing) {
    // figure out the correct rate of FRENS for this token
    tx = await facet.setGhstWethToken(
      ghstWethToken,
      StkGHSTWETH.address,
      ethers.BigNumber.from("12077243")
    );
    receipt = await tx.wait();
    if (!receipt.status) {
    ...
    tx = await facet.populateTransaction.setGhstWethToken(
      ghstWethToken,
      StkGHSTWETH.address,
      ethers.BigNumber.from("12077243")
    );
