...
async function deployDiamond (diamondName, owner, facets, ...otherArgs) {
  ...
  const deployedDiamond = await diamondFactory.deploy(owner, diamondCut, ...otherArgs)
...
async function upgradeDiamond ({
  ...
  if (selectorsToRemove && selectorsToRemove.length > 0) {
    ...
    diamondCut.push([ethers.constants.AddressZeo, FacetCutAction.Remove, selectorsToRemove])
  }
  ...
  const result = await diamondCutFacet.diamondCut(diamondCut, initFacetAddress, functionCall)
  console.log('------')
