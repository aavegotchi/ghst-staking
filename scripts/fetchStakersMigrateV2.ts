...
async function main() {
  ...
  while (blockNumber < currentBlockNumber) {
    ...
    const trackedTokenTransfersIn = await trackedToken.queryFilter(
      trackedTokenFilter,
      blockNumber,
      nextBlockNumber
    );
    console.log(
      `Got ${trackedTokenTransfersIn.length} transfers in from ${blockNumber} to ${nextBlockNumber}`
    );
  ...
  while (startIndex < stakers.size) {
    ...
    const subArr = (
      endIndex > stakers.size
        ? stakerArray.slice(startIndex)
        : stakerArray.slice(startIndex, endIndex)
    ) as string[];
